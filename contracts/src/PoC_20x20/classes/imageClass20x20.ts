...
export class NewImageSize20x20 extends Struct({
  width: Field,
  height: Field,
  bitmap: Provable.Array(Field, 1600),
}) {
  static from(bitmap: number[]) {
    return new NewImageSize20x20({
      width: Field.from(20),
      height: Field.from(20),
      bitmap: bitmap.map((number) => Field(number)),
    });
  }

  public invert(): NewImageSize20x20 {
    let invertedBitmap: Field[] = new Array<Field>(1600);
